...
function getCullingVolume(viewer) {
  ...
  const offset = Cesium.Cartesian3.multiplyByScalar(viewer.camera.direction, -10000.0, new Cesium.Cartesian3());
...
function onCameraChanged(viewer) {
  ...
  const sortedIntersectingTiles = intersectingTiles(tiles, cullingVolume, viewer, zoomLevel, viewer);
          ...
          .then(originalTileData => {
            ...
            Object.entries(bayGroups).forEach(([bayId, conductors]) => {
              ...
              conductors.forEach(conductor => {
                ...
                addSplineForPoints(tileId, conductorId, cartesian, color, primitiveMap, primitiveCollectionMap, viewer);
                if (conductors.length > 1) {
                  const firstPoints = conductors.map(conductor => conductor.cartesian[0]);
                  const endPoints = conductors.map(conductor => conductor.cartesian[10]);

                  addCrossarms(tileId, firstPoints, viewer, primitiveCollectionMap, primitiveMap);
                  addCrossarms(tileId, endPoints, viewer, primitiveCollectionMap, primitiveMap);
                }
              });
            });
            return { tileData };
  ...
  removePrimitiveForInvisibleTiles(newVisibleTileIds, primitiveCollectionMap, viewer);
}
...
function resetLastPickedPrimitiveColor() {
  if (lastPickedPrimitive && lastPickedPrimitiveOriginalColor) {
    ...
    } else if (lastPickedPrimitive.appearance && lastPickedPrimitive.appearance.material) {
...
function resetLastHoveredPrimitiveColor() {
  if (lastHoveredPrimitive && lastHoveredPrimitiveOriginalColor) {
    ...
    } else if (lastHoveredPrimitive.appearance && lastHoveredPrimitive.appearance.material) {
      // For other primitives that have an appearance and material, reset the material color
      lastHoveredPrimitive.appearance.material.uniforms.color = lastHoveredPrimitiveOriginalColor;
    }
  }
  // Clear the references after resetting the color
...
function removeConductorPoints(conductorId) {
  const pointsToRemove = viewer.entities.values.filter(entity => entity.id.startsWith(`point_${conductorId}_`));
...
function togglePointsVisibility(conductorId, isVisible) {
  let pointsEntities = viewer.entities.values.filter(entity => entity.id.startsWith(`point_${conductorId}_`));
...
async function onLeftClick(click) {
  const pickedObject = viewer.scene.pick(click.position);
  console.log(pickedObject);

  if (Cesium.defined(pickedObject) && pickedObject.primitive && pickedObject.id) {
    if (lastHoveredPrimitive && lastHoveredPrimitive === pickedObject.primitive) {
    ...
    if (typeof pickedObject.id._id === 'string' && pickedObject.id._id.startsWith('point_')) {
    ...
    } else if (typeof pickedObject.id === 'string' && pickedObject.id.startsWith(`"mgc_`)) {
      // console.log("mgs is working"+JSON.stringify(pickedObject.primitive.providedProperties, null, 2))
      // Add your code to handle pole selection
      highlightPrimitive(pickedObject.primitive);
      viewer.selectedEntity = new Cesium.Entity({
        name: `MGC Details`,
        description: createInfoBox(pickedObject.primitive.providedProperties, ['color']),
        id: pickedObject.id
      });
    } else if (typeof pickedObject.id === 'string' && pickedObject.id.startsWith(`"vi_`)) {
      highlightPrimitive(pickedObject.primitive);
      viewer.selectedEntity = new Cesium.Entity({
        name: `Vegetation Intrusion Details`,
        description: createInfoBox(pickedObject.collection.providedProperties, ['color']),
        id: pickedObject.id
      });
    } else if (typeof pickedObject.id === 'string' && pickedObject.id.startsWith(`"sicb_`)) {
      highlightPrimitive(pickedObject.primitive);
      viewer.selectedEntity = new Cesium.Entity({
      ...
      try {
        ...
        const conductorInfo = await fetchConductorInfo({ x, y }, conductorId, zoom);
        if (conductorInfo) {
          viewer.selectedEntity = new Cesium.Entity({
...
function highlightPoint(clickedPointId) {
  ...
  if (currentlySelectedPointId === clickedPointId) {
    ...
    if (currentlySelectedPointId) {
      var previousPointEntity = viewer.entities.getById(currentlySelectedPointId);
...
function updatePointPosition(direction) {
  if (currentlySelectedPointId) {
    ...
    if (pointEntity) {
      let currentPosition = Cesium.Cartesian3.clone(pointEntity.position.getValue(Cesium.JulianDate.now()));
      let newPosition;
      ...
      const wasHighlighted = lastPickedPrimitive === primitiveMap.get(currentConductorId);
      ...
      if (pointsSet && currentlySelectedPointIndex >= 0) {
        ...
        addSplineForPoints(currentTileId, currentConductorId, pointsSet, primitiveMap, primitiveCollectionMap, viewer);
...
function highlightHover(primitive) {
  ...
  if (primitive instanceof Cesium.PointPrimitive) {
    ...
    // For other primitives with an appearance and material, clone the material color
    lastHoveredPrimitiveOriginalColor = primitive.appearance.material.uniforms.color.clone();
    // Set the highlight color
